...
class GRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        prompt_ids_1, prompt_mask_1 = [x["input_ids"] for x in inputs], [x["attention_mask"] for x in inputs]

        prompt_ids = torch.stack([torch.stack(row) for row in prompt_ids_1])
        prompt_mask = torch.stack([torch.stack(row) for row in prompt_mask_1])
        ...
        for i, (reward_func, reward_processing_class, reward_func_name) in enumerate(
            ...
            with profiling_context(self, reward_func_name):
                if isinstance(reward_func, nn.Module):  # Module (no PretrainedModel) for compat with compiled models
                    ...
                    output_reward_func, predict_text = reward_func(
        ...
        self._textual_logs["prompt"].extend(gather_object(predict_text))
        self._textual_logs["completion"].extend(gather_object(reward_kwargs['expected_text']))
